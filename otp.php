...
<?PHP
    if(isset($_POST['register']))
    {
        if($_POST['otp']==$_SESSION['token'])
        {
            ...
            $sql="INSERT INTO `user` (`Name`, `BuisnessName`, `BuisnessType`, `GSTIN`, `Category`, `Email`, `Password`, `PhoneNumber`) VALUES ('$name', '$businessName', '$businessType', '$gstNumber', '$category', '$email', '$hpass', '$phoneNumber')";
            $result=mysqli_query($con,$sql);
            session_destroy();
            echo "<script>alert('Account Created Successfully, Kindly LogIn!')</script>";
