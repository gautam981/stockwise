<?PHP
  ...
  // Retrieve product ID from POST or GET parameters
  if(isset($_POST['id'])) {
    $_SESSION['current_product_id'] = $_POST['id'];
  } elseif(isset($_GET['id'])) {
    $_SESSION['current_product_id'] = $_GET['id'];
  }
  
  $productId = isset($_SESSION['current_product_id']) ? $_SESSION['current_product_id'] : null;
  
  // If no product ID in session, get the first product in database
  if(!$productId) {
    $sql = "SELECT ProductID FROM `product` ORDER BY ProductID LIMIT 1";
  ...
  // If we have a product ID, get product details
  if($productId) {
    ...
    if(!$product) {
      // Product not found, try to get the first product
      $sql = "SELECT * FROM `product` ORDER BY ProductID LIMIT 1";
      $result = mysqli_query($con, $sql);
      $product = mysqli_fetch_assoc($result);
      ...
      if($product) {
        ...
        echo "<script>alert('No products found in the database!'); window.location.href='products.php';</script>";
        exit();
      }
    }
  } else {
  ...
  // Get all product IDs for the product selector dropdown
  $allProductsQuery = "SELECT ProductID, Name FROM `product` ORDER BY Name";
...
<head>
  ...
  <title>StockWise: Manage Product</title>
  <!-- Bootstrap CSS -->
    ...
    <div id="content">
      <div class="container-fluid">
        ...
        <div class="product-selector mb-4">
          <form id="productSelectForm" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <div class="row align-items-center">
              <div class="col-md-8">
                <div class="form-group mb-0">
                  <label for="productSelect" class="form-label">Selected Product</label>
                  <select class="form-control" id="productSelect" name="id" onchange="this.form.submit()">
                    <?php
                      mysqli_data_seek($allProductsResult, 0); // Reset result pointer
                      while($productOption = mysqli_fetch_assoc($allProductsResult)) {
                        $selected = ($productOption['ProductID'] == $productId) ? 'selected' : '';
                        echo "<option value='" . $productOption['ProductID'] . "' $selected>" . 
                            htmlspecialchars($productOption['Name']) . " (ID: " . $productOption['ProductID'] . ")</option>";
                      }
                    ?>
                  </select>
                </div>
        ...
        <div class="card">
          <div class="card-header">
            <h5><i class="bi bi-pencil-square me-2"></i>Edit Product Details</h5>
          </div>
          <div class="card-body">
            <form id="editProductForm" class="pb-3" method="POST" action="">
              <!-- Hidden field to preserve product ID -->
              <input type="hidden" name="id" value="<?php echo $productId; ?>">
              
              <div class="row">
                <div class="col-md-6 form-group">
                  <label for="productName" class="form-label">Product Name<span class="required-indicator">*</span></label>
                  <input type="text" class="form-control animated-input" id="productName" placeholder="Enter product name" required name="name" value="<?php echo htmlspecialchars($product['Name']); ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label for="productSKU" class="form-label">SKU<span class="required-indicator">*</span></label>
                  <input type="text" class="form-control animated-input" id="productSKU" placeholder="Enter product SKU" required name="sku" value="<?php echo htmlspecialchars($product['SKU']); ?>" readonly>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-6 form-group">
                  ...
                  <div class="input-group">
                    ...
                    <input type="number" class="form-control animated-input" id="sellingPrice" placeholder="0.00" step="0.01" min="0" required name="sp" value="<?php echo htmlspecialchars($product['SP']); ?>">
                  </div>
                </div>
                <div class="col-md-6 form-group">
                  ...
                  <div class="input-group">
                    <span class="input-group-text"><i class="bi bi-currency-rupee"></i></span>
                    <input type="number" class="form-control animated-input" id="purchasePrice" placeholder="0.00" step="0.01" min="0" required name="pp" value="<?php echo htmlspecialchars($product['PP']); ?>">
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-6 form-group">
                  ...
                  <input type="number" class="form-control animated-input" id="productInventory" placeholder="0" min="0" required name="inventory" value="<?php echo htmlspecialchars($product['Inventory']); ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label for="productLocation" class="form-label">Location</label>
                  <input type="text" class="form-control animated-input" id="productLocation" placeholder="Enter storage location" name="loc" value="<?php echo htmlspecialchars($product['Location']); ?>">
                </div>
              </div>
              
              <div class="form-group">
                <label for="productDescription" class="form-label">Description</label>
                <textarea class="form-control animated-input" id="productDescription" rows="3" placeholder="Enter product description" name="desc"><?php echo htmlspecialchars($product['Description']); ?></textarea>
              </div>
              
              <div class="d-flex justify-content-end">
                <button type="button" class="btn btn-light me-3" onclick="window.location.href='products.php';">
...
<?PHP
  ...
  {
    // Make sure we still have the product ID
    if(isset($_POST['id'])) {
      $updateProductId = $_POST['id'];
      $name = $_POST['name'];
      $sp = $_POST['sp'];
      $pp = $_POST['pp'];
      ...
      if($inventory > 0)
      {
        if($sp > $pp)
        {
          ...
          if($result) {
            ...
            // Redirect to the same page with the updated product ID to ensure proper refresh
            echo "<script>window.location.href='products.php';</script>";
            exit();
          } else {
            echo "<div class='alert alert-danger' role='alert'>
                    Failed to update product: " . mysqli_error($con) . "
                  </div>";
          }
        }
        else
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      function adjustLayout() {
        ...
        if (isVerySmall) {
          ...
          sidebar.style.maxWidth = '160px</div>
       </div>
     </div>
   </div>
 </div>
